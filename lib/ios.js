  ...
  var Efte = {
    ...
    send_message: function(method, args, callback) {
      ...
      for(var index in callbacks) {
        console.log('---- send_message ' + index + ' ' + callbacks[index]);
      }
    ...
    callback: function(callbackId, retValue) {
      try {
        ...
        console.log('---- callback id: ' + callbackId + ' callback: ' + callback);
        if (!callback) return;
    ...
    ajax: function(opts) {
      ...
      var contentTypeToDataType = function(contentType) {
        ...
        return contentType && (contentType == jsonType ? 'json' : contentType == htmlType ? 'html' : xmlTypeRE.test(contentType) && 'xml') || 'text';
      }
    ...
    action: {
      reloadPage: function() {
        if (Efte.getPlatform() === 'android') {
            Efte.send_message('reloadPage');
        }
        else {
          location.reload();
        }
    ...
    getEnv: function(callback) {
      var cid = Efte.send_message('getEnv', {}, function(env) {
        window.AppEnv = env;
    ...
    getPlatform: function() {
      ...
      if(userAgent.indexOf('efte-for-android') != -1) {
        return 'android';
      }
      if(userAgent.indexOf('efte-for-ios') != -1) {
        return 'ios';
      }
      return '';
    },
     ...
     */
     ...
     // deprecated 因为参数过多，现在使用date 这个api
    ...
    /*
      type: {String} date|time|datetime(default)
      default: {String} 2014-11-11 12:12:12(default) | 2014-11-11 | 12:12:12
      minuteInterval: 分钟间隔，必须能被60整除，例如：1,10,15,30
      minDate: 最小的有效日期，以1970年开始的毫秒数
      maxDate: 最大的有效日期，以1970年开始的毫秒数
    */
    date: function(options, callback) {
      Efte.send_message('datepicker', options, function(result) {
        (typeof callback == 'function') && callback(result.value);
      });
    },
    ...
    showPhoto: function(data, delCallback) {
      var cid = Efte.send_message('showPhoto', data, function(retValue) {
    ...
    ga: {
      sendEvent: function(category, action, label, value, success, error) {
        var cid = Efte.send_message('ga_sendEvent', {
          category: category,
          action: action,
          label: label,
          value: value
      ...
      sendView: function(viewName, success, error) {
        var cid = Efte.send_message('ga_sendView', {
          viewName: viewName
        }, function(data) {
      ...
      sendTiming: function(category, interval, name, label, success, error) {
        var cid = Efte.send_message('ga_sendTiming', {
          category: category,
          interval: interval,
          name: name,
          label: label
      ...
      sendException: function(description, fatal, success, error) {
        var cid = Efte.send_message('ga_sendException', {
      ...
      setCustomDimension: function(dimentionIndex, value, success, error) {
        var cid = Efte.send_message('ga_setCustomDimension', {
      ...
      setCustomMetric: function(metricIndex, value, success, error) {
        var cid = Efte.send_message('ga_setCustomMetric', {
    ...
    cache: {
      save: function(key, value, opts) {
        ...
        var cid = Efte.send_message('cacheSave', params, function(data) {
          delete callbacks[cid];
          if (!data || !data.status) {
      ...
      load: function(key, opts) {
        opts = opts || {};
        var cid = Efte.send_message('cacheLoad', {
          key: key
        }, function(data) {
          delete callbacks[cid];
          if (!data || !data.status) {
      ...
      remove: function(key, opts) {
        opts = opts || {};
        var cid = Efte.send_message('cacheDelete', {
          key: key
        }, function(data) {
          delete callbacks[cid];
          if (!data || !data.status) {
      ...
      removeAll: function(opts) {
        opts = opts || {};
        var cid = Efte.send_message('cacheClear', {}, function(data) {
          delete callbacks[cid];
          if (!data || !data.status) {
            callbackDelegate(opts.error, [data.message || 'cache清除失败']);
            return;
          }
          callbackDelegate(opts.success, [data]);
        });
    ...
    push: {
      getInfo: function(success, error) {
        console.log('---- in efte.push.getInfo');
        var cid = Efte.send_message('push_getInfo', {}, function(data) {
          console.log('---- in efte.push.getInfo callback: ' + JSON.stringify(data));
          delete callbacks[cid];
          if (!data || !data.status) {
            callbackDelegate(error, [data]);
            return;
          }
          callbackDelegate(success, [data]);
        })
      },
      getExtra: function(success, error) {
        var cid = Efte.send_message('push_getExtra', {}, function(data) {
          delete callbacks[cid];
          if (!data || !data.status || !data.extra) {
            callbackDelegate(error, [data]);
            return;
          }
          callbackDelegate(success, [data.extra]);
        });
      }
    }
  };
