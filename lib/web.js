(function() {
  ...
  var Efte = {
    ...
    ga: 'sendEvent sendView sendTiming sendException setCustomDimension setCustomMetric'.split(' ').reduce(function(o, name) {
      o[name] = function() {
        console.log('ga:', name, [].slice.call(arguments));
      }
      return o;
    }, {}),
    ...
    cache: {
      save: function (key, value, opt) {
        localStorage.setItem(key, JSON.stringify(value));
      },
      load: function (key, opt) {
        var value = localStorage.getItem(key);
        if (value == null) {
          opt.error();
        }
        else {
          opt.success(JSON.parse(value));
        }
      },
      remove: function (key) {
